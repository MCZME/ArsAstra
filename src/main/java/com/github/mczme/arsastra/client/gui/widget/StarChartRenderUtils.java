...
public class StarChartRenderUtils {
    ...
    // 纹理资源定义
    public static final ResourceLocation PARCHMENT_TEXTURE = ResourceLocation.fromNamespaceAndPath(ArsAstra.MODID,
    ...
    public static void drawGlobalHatching(PoseStack poseStack, float x, float y, float w, float h, float worldScale,
            float worldOffsetX, float worldOffsetY, int color) {
        ...
        if (shader.getUniform("UVScale") != null)
            shader.getUniform("UVScale").set(worldScale);
        if (shader.getUniform("UVOffset") != null)
            shader.getUniform("UVOffset").set(worldOffsetX, worldOffsetY);
    ...
    public static void drawSolidCircle(PoseStack poseStack, Vector2f center, float radius) {
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            buffer.addVertex(matrix, center.x + (float) Math.cos(angle) * radius,
                    center.y + (float) Math.sin(angle) * radius, 0);
        }
     ...
     */
    @SuppressWarnings("null")
    public static void drawHatchedPolygonFilled(PoseStack poseStack, List<Vector2f> points, int color, float uvScale,
    ...
    public static void drawDynamicCircle(PoseStack poseStack, Vector2f center, float radius, int color,
            float baseWidth) {
    ...
    @SuppressWarnings("null")
    public static void drawMonochromeIcon(PoseStack poseStack, TextureAtlasSprite sprite, Vector2f center, float size,
            int color) {
        if (sprite == null)
            return;
    ...
    private static void drawRibbonPath(PoseStack poseStack, List<Vector2f> vertices, boolean closed, float width,
            int color, float alphaMult) {
        if (vertices.size() < 2)
            return;
    ...
    private static void addSegmentVertices(BufferBuilder buffer, Matrix4f matrix, Vector2f p1, Vector2f p2, float nx,
            float ny, float width, float r, float g, float b, float a) {
    ...
    // 简单的圆形 Join (使用扇形构建)
    private static void addJoinVertices(BufferBuilder buffer, Matrix4f matrix, Vector2f center, float radius, float r,
    ...
    private static void drawRibbonCircle(PoseStack poseStack, Vector2f center, float radius, float width, int color,
            float alphaMult) {
        int segments = 64;
        Tesselator tesselator = Tesselator.getInstance();
        BufferBuilder buffer = tesselator.begin(VertexFormat.Mode.TRIANGLES, DefaultVertexFormat.POSITION_COLOR);
